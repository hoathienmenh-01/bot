...
SETTING_GROUPS = [
    ...
    {
        "title": "4. Binance Pay tự động / Binance ID / USDT ví ngoài",
        "desc": "Binance Pay merchant API key/secret + webhook HTTPS dùng để tự xác nhận. Binance ID và địa chỉ USDT ví ngoài chỉ là luồng thủ công nếu không có API/webhook riêng.",
...
SETTING_META: dict[str, dict[str, str]] = {
    ...
    "BINANCE_PAY_ENABLED": {"label": "Bật Binance Pay tự động", "help": "Bật khi có Binance Pay merchant API key/secret và webhook HTTPS public. Khi bật, nạp qua Binance Pay có thể tự cộng ví bằng webhook.", "placeholder": "false"},
    "BINANCE_PAY_API_KEY": {"label": "Binance Pay Merchant API key", "help": "API key của Binance Pay merchant để tạo order thanh toán tự động.", "placeholder": ""},
    "BINANCE_PAY_SECRET_KEY": {"label": "Binance Pay Secret key", "help": "Secret key dùng để ký request và xác minh webhook Binance Pay. Không chia sẻ.", "placeholder": ""},
    ...
    "BINANCE_PAY_WEBHOOK_URL": {"label": "Webhook Binance Pay tự động", "help": "URL HTTPS public để Binance gọi về. Ví dụ: https://domain-cua-ban/webhook/binance. Không có webhook thì Binance Pay không tự xác nhận được.", "placeholder": "https://domain-cua-ban/webhook/binance"},
    "BINANCE_PAY_ID": {"label": "Binance ID nhận tiền thủ công", "help": "Chỉ dùng khi không dùng Binance Pay merchant. Khách chuyển xong admin xác nhận bằng mã NAP/TXID.", "placeholder": "750113881"},
    ...
    "USDT_BEP20_ADDRESS": {"label": "Địa chỉ ví USDT nhận thủ công", "help": "Địa chỉ ví ngoài để bot tạo QR. Hiện luồng này cần admin xác nhận TXID nếu chưa tích hợp API blockchain riêng.", "placeholder": "0x... / T..."},
...
class AdminRequestHandler(BaseHTTPRequestHandler):
    ...
    def _page(self, title_key: str, body: str, *, active: str | None = None, alert: str = "", error: str = "") -> str:
        ...
        nav_parts = []
        for idx, (group_title, items) in enumerate(NAV_GROUPS):
            links = []
            group_has_active = False
            for path, key in items:
                if role_can_read(role, path):
                    group_has_active = group_has_active or (active == path)
                    links.append(
            ...
            if links:
                open_attr = " open" if group_has_active or idx == 0 else ""
                nav_parts.append(
                    f'<details class="nav-group" data-nav-group="{idx}"{open_attr}>'
                    f'<summary class="nav-heading"><span>{esc(group_title)}</span><span class="nav-caret">⌄</span></summary>'
                    f'<div class="nav-links">{"".join(links)}</div></details>'
                )
        nav = "".join(nav_parts)
